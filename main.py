...
# RAM Önbelleği: Çekilen veri burada saklanır, TKGM kotası asla boş yere tüketilmez!
CACHE = {}
...
async def fetch_tkgm(path: str):
    if path in CACHE:
        print(f"[CACHE] Hafızadan getirildi: {path}")
        return CACHE[path]
    ...
    async with httpx.AsyncClient(verify=False, follow_redirects=True, timeout=12.0) as client:
        for base in CBS_BASE_URLS:
            ...
            try:
                ...
                print(f"[TKGM İSTEK] {url} -> Durum: {res.status_code}")
                if res.status_code == 200:
                    data = res.json()
                    CACHE[path] = data  # Hafızaya kaydet
                    return data
            except Exception as e:
                print(f"[TKGM HATA] {url} -> {e}")
                continue
...
async def get_iller():
    ...
    async with httpx.AsyncClient(verify=False, timeout=8.0) as client:
        try:
            ...
            if res.status_code == 200:
                data = res.json()
                iller = [{"id": f["properties"]["id"], "ad": f["properties"]["text"]} for f in data.get("features", [])]
...
async def get_ilceler(il_id: int):
    ...
    ilceler = [{"id": f["properties"]["id"], "ad": f["properties"]["text"]} for f in data.get("features", [])]
...
async def get_mahalleler(ilce_id: int):
    ...
    mahalleler = [{"id": f["properties"]["id"], "ad": f["properties"]["text"]} for f in data.get("features", [])]
...
# 5. Haritadan Tıklanan Koordinat
